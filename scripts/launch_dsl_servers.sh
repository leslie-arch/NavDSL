...
export CLASSES_PATH=${CLASSES_PATH:-navdsl/vlm/classes.txt}
...
tmux send-keys -t ${session_name}:0.0 "${VLFM_PYTHON} -m navdsl.vlm.grounding_dino --port ${GROUNDING_DINO_PORT}" C-m
tmux send-keys -t ${session_name}:0.1 "${VLFM_PYTHON} -m navdsl.vlm.blip2itm --port ${BLIP2ITM_PORT}" C-m
tmux send-keys -t ${session_name}:0.2 "${VLFM_PYTHON} -m navdsl.vlm.sam --port ${SAM_PORT}" C-m
tmux send-keys -t ${session_name}:0.3 "${VLFM_PYTHON} -m navdsl.vlm.yolov7 --port ${YOLOV7_PORT}" C-m
