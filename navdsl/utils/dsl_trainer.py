...
@baseline_registry.register_trainer(name="dsl")
class DSLTrainer(PPOTrainer):
    envs: VectorEnv

    def load_checkpoint(self, checkpoint_path, *args, **kwargs):
        """
        Override the parent class load_checkpoint method to handle PyTorch 2.6+
        security restrictions with omegaconf.dictconfig.DictConfig.
        """
        # Add omegaconf.dictconfig.DictConfig to the safe globals list
        torch.serialization.add_safe_globals(['omegaconf.dictconfig.DictConfig'])

        # If weights_only is not explicitly set, use False for backward compatibility
        if 'weights_only' not in kwargs:
            kwargs['weights_only'] = False

        return super().load_checkpoint(checkpoint_path, *args, **kwargs)
    ...
    def _eval_checkpoint(
        ...
        # )
        # 用于根据配置文件的 _target_ 字段动态实例化 Python 对象（类实例、函数或函数调用）
        # 配置中必须包含 _target_ 字段，指向要实例化的类或函数的路径（如 module.ClassName）
        self._init_envs(config, is_eval=True)

        self._agent = self._create_agent(None)
        #print(f"Agent.actor_critic Available attributes: {dir(self._agent.actor_critic)}")
        # action_shape, discrete_actions = get_action_space_info(self._agent.policy_action_space) # v0.2.x
        action_shape, discrete_actions = get_action_space_info(self._agent.actor_critic._action_space) # v0.3.x

        if self._agent.actor_critic.should_load_agent_state:
            print(f"agent actor_critic load state_dict: {checkpoint_path}\n{ckpt_dict}")
